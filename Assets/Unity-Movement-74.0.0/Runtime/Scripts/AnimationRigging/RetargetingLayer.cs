...
namespace Oculus.Movement.AnimationRigging
{
        ...
        IOVRSkeletonProcessorAggregator
    {
        ...
        private int _cachedProxyChangeCount = -1;   // Proxy 재생성 여부 추적

        public void FillTransformArrays(
            List<Transform> sourceTransforms,
            List<Transform> targetTransforms,
            List<bool> shouldUpdatePositions,
            List<bool> shouldUpdateRotations,
            List<Quaternion> rotationOffsets,
            List<Quaternion> rotationAdjustments)
        {
            // ① 리스트를 재사용하기 위해 전부 Clear()
            sourceTransforms.Clear();
            targetTransforms.Clear();
            shouldUpdatePositions.Clear();
            shouldUpdateRotations.Clear();
            rotationOffsets.Clear();
            rotationAdjustments.Clear();

            // ② 한 번만(또는 Proxy가 바뀌었을 때만) Capacity 확보
            //    - ProxyChangeCount 가 바뀌면 본 개수가 달라질 수 있으므로 재확보
            const int kMaxHumanoidBones = (int)HumanBodyBones.LastBone; // 55
            if (sourceTransforms.Capacity < kMaxHumanoidBones ||
                _proxyTransformLogic.ProxyChangeCount != _cachedProxyChangeCount)
            {
                sourceTransforms.Capacity = kMaxHumanoidBones;
                targetTransforms.Capacity = kMaxHumanoidBones;
                shouldUpdatePositions.Capacity = kMaxHumanoidBones;
                shouldUpdateRotations.Capacity = kMaxHumanoidBones;
                rotationOffsets.Capacity = kMaxHumanoidBones;
                rotationAdjustments.Capacity = kMaxHumanoidBones;

                _cachedProxyChangeCount = _proxyTransformLogic.ProxyChangeCount;
            }
            ...
            for (int i = 0; i < numBones; ++i)
            {
                ...
                if (targetBoneData == null ||
                    !targetBoneData.CorrectionQuaternion.HasValue)
                {
                    continue; // 매핑 실패 → skip
                }
                ...
                targetTransforms.Add(targetBoneData.OriginalJoint);
                shouldUpdatePositions.Add(false); // 기본값, UpdateAdjustments 에서 결정
                shouldUpdateRotations.Add(false); //      "
                rotationOffsets.Add(targetBoneData.CorrectionQuaternion.Value);
                rotationAdjustments.Add(Quaternion.identity);
            }
