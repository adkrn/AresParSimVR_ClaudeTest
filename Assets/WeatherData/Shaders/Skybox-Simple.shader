Shader "Custom/Skybox/Simple"
{
    Properties
    {
    	...
    	// ── 새 태양 각도 ──
	    _SunAzimuth   ("Sun Azimuth  (° 0-360)", Range(0,360)) = 0      // 경도 : 동쪽 0°, 시계 반대
	    _SunElevation ("Sun Elevation(° 0~360)", Range(0,360)) = 45    // 위도 : 지평선 0°, 위로 +
    	
    	
    	[Toggle(_SHOW_SUN)]   _ShowSun   ("Show Sun"  , Float) = 1
        [Toggle(_SHOW_MOON)]  _ShowMoon  ("Show Moon" , Float) = 1
        [Toggle(_SHOW_STARS)] _ShowStars ("Show Stars", Float) = 1
    	...
    	// ─ Cirrus (얇은 운무) ─────────────────────────────
		_CloudMap          ("Cirrus Noise",       2D)    = "white" {}
		_CloudAlpha        ("Cirrus Alpha",       Range(0,1)) = 0.8
		_CloudCoverage     ("Cirrus Coverage",    Range(0,1)) = 0.6
		_CloudAltitude     ("Cirrus Altitude",    Range(0,1)) = 0.2
		_CloudColor        ("Cirrus Color",       Color) = (1,1,1,1)
		_CloudColorPower   ("Cirrus Color Power", Range(0,4)) = 1
		_CloudAnimation    ("Cirrus Scroll (XY)", Vector) = (0.02,0.01,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			uniform float  _SunAzimuth;    // degrees
			uniform float  _SunElevation;  // degrees
            
            uniform half   _ShowSun;
			uniform half   _ShowMoon;
			uniform half   _ShowStars;
			...
			inline float3 SphericalToDirection (float azimuthDeg, float elevationDeg)
			{
			    float2 rad = radians(float2(azimuthDeg, elevationDeg)); // deg→rad
			    float  cosElev = cos(rad.y);
			    return float3(
			        cosElev * cos(rad.x),   // x
			        sin(rad.y),             // y (Unity 기준 ↑)
			        cosElev * sin(rad.x)    // z
			    );
			}
...
//#endif
            ...
            fixed4 frag(v2f i) : COLOR 
            {
				...
				{		
					...
					moonSampler *= (_ShowMoon * _MoonIntensity);
					
					float starsBehindMoon = 1 - clamp((moonArea * 5), 0, 1);

            		float3 starsMap = texCUBE(_Stars, i.starPos.xyz);
					float4 nightSky = float4(((_ShowStars * _StarsIntensity * 50) * starsMap.rgb), 1)* starsBehindMoon;
					...
					float3 _SunDir = SphericalToDirection(_SunAzimuth, _SunElevation);
					_SunDir *= 1.001;
					half eyeCos = dot(_SunDir, ray);
					half eyeCos2 = eyeCos * eyeCos;
					half mie = getMiePhase(eyeCos, eyeCos2,y);
					skyColor += (_ShowSun * _SunIntensity) * mie * _SunColor;
